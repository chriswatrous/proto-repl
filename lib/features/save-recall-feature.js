...
(function() {
  ...
  module.exports = SaveRecallFeature = (function() {
    ...
    SaveRecallFeature.prototype.clearSavedValues = function() {
      var editor;
      this.protoRepl.executeCode("(proto-repl.saved-values/clear-saved-values!)", {
        displayInRepl: false
      });
      if (editor = atom.workspace.getActiveTextEditor()) {
    ...
    SaveRecallFeature.prototype.fetchAndDisplaySavedValues = function() {
      return this.protoRepl.executeCode("(proto-repl.saved-values/saved-values)", {
        displayInRepl: false,
        resultHandler: (function(_this) {
          return function(result, options) {
            ...
            if (result.error) {
              _this.protoRepl.stderr("Error polling for saved values " + result.error);
              return;
            }
            ...
            for (i = 0, len = uniqsToTrees.length; i < len; i++) {
              ...
              if (foundRange = _this.protoRepl.EditorUtils.findEditorRangeContainingString(uniq)) {
                editor = foundRange[0], range = foundRange[1];
                results.push(_this.protoRepl.repl.displayInline(editor, range, tree));
              } else {
                results.push(void 0);
              }
            }
