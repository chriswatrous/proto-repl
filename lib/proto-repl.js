...
(function() {
  ...
  module.exports = {
    ...
    toggle: function(projectPath) {
      ...
      if (this.repl === null) {
        this.repl = new Repl(this.extensionsFeature);
    ...
    prepareRepl: function(repl) {
      repl.consumeInk(this.ink);
      repl.onDidClose((function(_this) {
        return function() {
          _this.repl = null;
          return _this.emitter.emit('proto-repl:closed');
        };
      })(this));
      repl.onDidStart((function(_this) {
        return function() {
          _this.emitter.emit('proto-repl:connected');
          if (atom.config.get("proto-repl.refreshOnReplStart")) {
            return _this.refreshNamespaces();
          }
        };
      })(this));
      return repl.onDidStop((function(_this) {
        return function() {
          _this.extensionsFeature.stopExtensionRequestProcessing();
          return _this.emitter.emit('proto-repl:stopped');
        };
      })(this));
    },
    onDidConnect: function(callback) {
      return this.emitter.on('proto-repl:connected', callback);
    },
    onDidClose: function(callback) {
      return this.emitter.on('proto-repl:closed', callback);
    },
    onDidStop: function(callback) {
      return this.emitter.on('proto-repl:stopped', callback);
    },
    ...
    remoteNReplConnection: function() {
      ...
      confirmCallback = (function(_this) {
        return function(arg) {
          ...
          if (!_this.repl) {
            _this.repl = new Repl(_this.extensionsFeature);
    ...
    running: function() {
      var ref1;
      return (ref1 = this.repl) != null ? ref1.running() : void 0;
    },
    getReplType: function() {
      var ref1;
      return (ref1 = this.repl) != null ? ref1.getType() : void 0;
    },
    isSelfHosted: function() {
      var ref1;
      return (ref1 = this.repl) != null ? ref1.isSelfHosted() : void 0;
    },
    clearRepl: function() {
      var ref1;
      return (ref1 = this.repl) != null ? ref1.clear() : void 0;
    },
    info: function(text) {
      var ref1;
      return (ref1 = this.repl) != null ? ref1.info(text) : void 0;
    },
    stderr: function(text) {
      var ref1;
      return (ref1 = this.repl) != null ? ref1.stderr(text) : void 0;
    },
    stdout: function(text) {
      var ref1;
      return (ref1 = this.repl) != null ? ref1.stdout(text) : void 0;
    },
    doc: function(text) {
      var ref1;
      return (ref1 = this.repl) != null ? ref1.doc(text) : void 0;
    },
    interrupt: function() {
      var ref1;
      return (ref1 = this.repl) != null ? ref1.interrupt() : void 0;
    },
    quitRepl: function() {
      var ref1;
      return (ref1 = this.repl) != null ? ref1.exit() : void 0;
    },
    registerCodeExecutionExtension: function(name, callback) {
      return this.extensionsFeature.registerCodeExecutionExtension(name, callback);
    },
    ...
    autoEvalCurrent: function() {
      ...
      if (!atom.config.get('proto-repl.showInlineResults')) {
        this.stderr("Auto Evaling is not supported unless inline results is enabled");
        return null;
      }
      if (!this.ink) {
        this.stderr("Install Atom Ink package to use auto evaling.");
        return null;
      }
      if (editor = atom.workspace.getActiveTextEditor()) {
        if (editor.protoReplAutoEvalDisposable) {
          return this.stderr("Already auto evaling");
        } else {
    ...
    getClojureVarUnderCursor: function(editor) {
      ...
      if (word === "") {
        this.stderr("This command requires you to position the cursor on a Clojure var.");
        return null;
      } else {
        return word;
      }
    ...
    refreshResultHandler: function(callback, result) {
      if (result.value) {
        ...
        return this.stderr("Refresh Warning: " + result.error);
      }
    },
    refreshNamespaces: function(callback) {
      ...
      if (this.isSelfHosted()) {
        return this.stderr("Refreshing not supported in self hosted REPL.");
      } else {
        this.info("Refreshing code...\n");
    ...
    superRefreshNamespaces: function(callback) {
      ...
      if (this.isSelfHosted()) {
        ...
        this.info("Clearing all and then refreshing code...\n");
    ...
    loadCurrentFile: function() {
      ...
      if (editor = atom.workspace.getActiveTextEditor()) {
        if (this.isSelfHosted()) {
          return this.stderr("Loading files is not supported yet in self hosted REPL.");
        } else {
    ...
    runTestsInNamespace: function() {
      ...
      if (editor = atom.workspace.getActiveTextEditor()) {
        if (this.isSelfHosted()) {
          return this.stderr("Running tests is not supported yet in self hosted REPL.");
        } else {
            ...
            resultHandler: (function(_this) {
              return function(value) {
                ...
                if (value.value) {
                  ...
                  if (msg) {
                    return _this.doc(msg);
                  } else {
                    return _this.stderr("No doc found for " + varName);
                  }
                } else if (value.error) {
                  return _this.stderr(value.error);
                } else {
    ...
    printVarCode: function() {
      ...
      if (editor = atom.workspace.getActiveTextEditor()) {
        if (varName = this.getClojureVarUnderCursor(editor)) {
          if (this.isSelfHosted()) {
            return this.stderr("Showing source code is not yet supported in self hosted REPL.");
          } else {
    ...
    listNsVars: function() {
      ...
      if (editor = atom.workspace.getActiveTextEditor()) {
        if (nsName = this.getClojureVarUnderCursor(editor)) {
          if (this.isSelfHosted()) {
            return this.stderr("Listing namespace functions is not yet supported in self hosted REPL.");
          } else {
    ...
    openFileContainingVar: function() {
      ...
      if (this.isSelfHosted()) {
        return this.stderr("Opening files containing vars is not yet supported in self hosted REPL.");
      } else {
        if (editor = atom.workspace.getActiveTextEditor()) {
            ...
            return this.executeCodeInNs(text, {
              ...
              resultHandler: (function(_this) {
                return function(result) {
                  var file, line, ref1;
                  if (result.value) {
                    _this.info("Opening " + result.value);
                    ...
                    return _this.stderr("Error trying to open: " + result.error);
                  }
                };
              })(this)
            });
